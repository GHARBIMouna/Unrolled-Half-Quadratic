...
parser.add_argument('--function', type=str, required=True,
                    help="Hybrid or test, convex, non-convex or hybrid, double check parameters.")
parser.add_argument('--Dataset', type=str, required=True, help="Dataset name")
parser.add_argument('--file_name', type=str, required=False,
                    help="file_name to save gridsearch results")
...
def MM_quadratic(delta_s, lamda, delta_s1, lamda1, H, L, x_true, x_degraded, x0, N, p, iterations, mode):
    ...
    a = torch.mm(torch.transpose(H, 0, 1), H) + lamda*torch.mm(torch.mm(torch.transpose(
        L, 0, 1), diag), L) + lamda1*torch.mm(torch.mm(torch.transpose(L, 0, 1), diag1), L)
    grad = torch.mm(torch.transpose(H, 0, 1), torch.mm(H, x0_b)-x_degraded) + lamda*torch.mm(torch.mm(torch.mm(
        torch.transpose(L, 0, 1), diag), L), x0_b) + lamda1*torch.mm(torch.mm(torch.mm(torch.transpose(L, 0, 1), diag1), L), x0_b)
    ...
    if mode == "SC":
        while (((torch.linalg.norm(x1_b-x0_b)**2)/(torch.linalg.norm(x0_b)**2)) > 1e-5) & (number_iterations < iterations):
            ...
            a = torch.mm(torch.transpose(H, 0, 1), H) + lamda*torch.mm(torch.mm(torch.transpose(
                L, 0, 1), diag), L)+lamda1*torch.mm(torch.mm(torch.transpose(L, 0, 1), diag1), L)
            grad = torch.mm(torch.transpose(H, 0, 1), torch.mm(H, x1_b)-x_degraded) + lamda*torch.mm(torch.mm(torch.mm(
                torch.transpose(L, 0, 1), diag), L), x1_b) + lamda1*torch.mm(torch.mm(torch.mm(torch.transpose(L, 0, 1), diag1), L), x1_b)
            bk = torch.mm(torch.inverse(a), grad)
            x0_b = x1_b
            x1_b = x1_b-bk
            mse = loss(x1_b, x_true)
                ...
                for i in range(num_signals):
                    if args.function in ["Gridsearch_cvx", "Gridsearch_ncvx", "Gridsearch_Hybrid"]:
                        H = np.load(os.path.join(
                            'Datasets'+args.Dataset, 'training', 'H', 'H_tr_'+str(i)+'.npy'), allow_pickle=True)
                        H = torch.from_numpy(H).type(torch.cuda.DoubleTensor)
                        x_true = torch.unsqueeze(torch.tensor(np.load(os.path.join(
                            'Datasets', args.Dataset, 'training', 'Groundtruth', 'x_Gr_tr_'+str(i)+'.npy'), allow_pickle=True)), 1).type(torch.cuda.DoubleTensor)
                        x_degraded = torch.unsqueeze(torch.tensor(np.load(os.path.join(
                            'Datasets'+args.Dataset+'training', 'Degraded', 'x_De_tr_'+str(i)+'.npy'), allow_pickle=True)), 1).type(torch.cuda.DoubleTensor)
                    if args.function in ["Test_cvx", "Test_ncvx", "Test_Hybrid"]:
                        H = np.load(os.path.join(
                            'Datasets'+args.Dataset, 'test', 'H', 'H_te_'+str(i)+'.npy'), allow_pickle=True)
                        H = torch.from_numpy(H).type(torch.cuda.DoubleTensor)
                        x_true = torch.unsqueeze(torch.tensor(np.load(os.path.join(
                            'Datasets', args.Dataset, 'test', 'Groundtruth', 'x_Gr_te_'+str(i)+'.npy'), allow_pickle=True)), 1).type(torch.cuda.DoubleTensor)
                        x_degraded = torch.unsqueeze(torch.tensor(np.load(os.path.join(
                            'Datasets'+args.Dataset+'test', 'Degraded', 'x_De_te_'+str(i)+'.npy'), allow_pickle=True)), 1).type(torch.cuda.DoubleTensor)

                    x0 = torch.zeros((2000, 1)).type(torch.cuda.DoubleTensor)
                    t0 = time.time()
                    MSE_values, x_pred, snr, tsnr, num_iterations = MM_quadratic(
                        delta_s_cvx, lamda_cvx, delta_s_ncvx, lamda_ncvx, H, L, x_true, x_degraded, x0, N, p, iterations, args.mode)
                    t1 = time.time()
...
file_object = open(os.path.join('Iterative_methods',
                   args.Dataset + '_'+args.file_name, "a"))
