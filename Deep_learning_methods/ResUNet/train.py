...
def main_worker(args, device, train_loader, val_loader, test_loader):
    ...
    if args.scheduler == "decay-lr":
        ...
        def lmbda(epoch): return 0.985
        scheduler = optim.lr_scheduler.MultiplicativeLR(
            optimizer, lr_lambda=lmbda)
    elif args.scheduler == "cyclic-lr":
        scheduler = optim.lr_scheduler.CyclicLR(
            optimizer, base_lr=args.base_lr, max_lr=args.lr, mode='triangular2', cycle_momentum=False)
    elif args.scheduler == "one-cycle-lr":
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=args.lr, steps_per_epoch=len(
            train_loader), epochs=args.epochs, cycle_momentum=False)
    else:  # constant-lr
        scheduler = None
    ...
    for epoch in range(args.epochs):
        train_loss = train(train_loader, device, net,
    ...
    plt.savefig(os.path.join(
        'Deep_learning_methods', 'ResUNet', 'ResUNet.png'))
    plt.close()
    # test
    loss, snr, tsnr, snr_std, tsnr_std = evaluate(
        test_loader, device, net, criterion, args)
...
def train(dataloader, device, net, optimizer, scheduler, criterion, epoch, args):
    ...
    progress = utilities.ProgressMeter(
        len(dataloader), [batch_time, losses], prefix="Epoch: [{}]".format(epoch))
...
def validate(dataloader, device, net, criterion, args):
    ...
    losses = utilities.AverageMeter('Loss', ':.4e')
    progress = utilities.ProgressMeter(
        len(dataloader), [batch_time, losses], prefix='Validation: ')
